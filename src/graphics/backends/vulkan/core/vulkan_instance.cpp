...
auto VulkanInstance::create(const CreateInfo &createInfo)
    -> std::expected<std::unique_ptr<VulkanInstance>, std::string>
{
    ...
    auto instancePtr = std::unique_ptr<VulkanInstance>(new VulkanInstance(
        instance,
        debugMessenger,
        validationEnabled,
        createInfo.platform
...
auto VulkanInstance::createSurface(void *windowHandle)
...
auto VulkanInstance::checkValidationLayerSupport(
...
auto VulkanInstance::getRequiredExtensions(const CreateInfo &createInfo)

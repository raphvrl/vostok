    ...
    -> std::expected<std::unique_ptr<VulkanDevice>, std::string>
{
    ...
    auto devicePtr = std::unique_ptr<VulkanDevice>(
        new VulkanDevice(device, createInfo.physicalDevice)
    );
...
auto VulkanDevice::initQueues() -> bool
{
...
auto VulkanDevice::createCommandPool() -> bool
{
    const QueueFamilyIndices &indices =
        m_physicalDevice->getQueueFamilyIndices();

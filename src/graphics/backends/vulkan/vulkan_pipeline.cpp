...
auto VulkanPipeline::Impl::pushRaw(const void *data, size_t size, u32 offset)
    -> std::expected<void, std::string>
{
...
auto VulkanPipeline::pushRaw(const void *data, size_t size, u32 offset)
    -> std::expected<void, std::string>
{
    return m_impl->pushRaw(data, size, offset);
}
